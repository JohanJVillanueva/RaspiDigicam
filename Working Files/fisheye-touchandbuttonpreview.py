...
def apply_fish_eye(image):
    """Apply a fish-eye effect to the image."""
    height, width = image.shape[:2]
    K = np.array([[width, 0, width // 2], [0, height, height // 2], [0, 0, 1]])
    D = np.array([0.2, 0.5, 0, 0])  # Distortion coefficients
    fish_eye_image = cv2.fisheye.undistortImage(image, K, D=D)
...
def capture_screenshot(frame):
    ...
    fish_eye_frame = apply_fish_eye(cropped_frame)
    ...
    cv2.putText(fish_eye_frame, current_time, (text_x, text_y), font, font_scale, font_color, font_thickness)

    # Save the image with effects to a file
    filename = f'/home/dsp/Pictures/{frame:03d}.jpg'
    cv2.imwrite(filename, fish_eye_frame)
